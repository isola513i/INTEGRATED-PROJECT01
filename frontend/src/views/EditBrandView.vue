...
const handleSubmitForm = async (brand) => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      flashForm.setMessage(`Brand name : ${formData.get('name').toLocaleUpperCase()} has been already existed`, "itbms-message m-1 p-1 px-2 bg-red-100 text-red-800 shadow  rounded-md")
...
<template>
  <div class="p-5">
    ...
    <BrandForm
    v-if="brand.id"
     @submitForm="handleSubmitForm" :brandEdit="brand" />
  </div>
</template>
