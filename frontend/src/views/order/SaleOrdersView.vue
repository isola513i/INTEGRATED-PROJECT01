...
const orders = ref([]); // Holds orders for the current tab and page
const pagination = ref({ page: 0, totalPages: 1 });
const loading = ref(true);
const error = ref(null);
const selectedTab = ref("New Orders"); // 'New Orders', 'Canceled Orders', 'All Orders'

// Map tab names to API status parameters
const tabStatusMap = {
	"New Orders": "new",
...
const loadOrders = async (page = 0) => {
	if (!auth.userId) return; // Should not happen due to route guard, but good practice
	...
	try {
		...
		pagination.value = {
			page: response.page, // API returns zero-based page index
			totalPages: response.totalPages,
		};
	} catch (e) {
		error.value = e.message || "An error occurred while fetching orders.";
		orders.value = []; // Clear orders on error
		pagination.value = { page: 0, totalPages: 1 }; // Reset pagination
	} finally {
		loading.value = false;
	}
};

// Watcher to reload data when tab changes, resetting to page 0
watch(selectedTab, () => {
	loadOrders(0);
});
...
const viewDetails = (orderId) => {
	...
	// No need to manually mark as viewed here, the BE does it when fetching detail
};

onMounted(() => {
	// Initial load for the default tab ('New Orders')
	loadOrders();
});
</script>
			...
			<div class="mb-6 border-b border-gray-200">
				<nav class="flex space-x-4" aria-label="Tabs">
					<button
						...
						:class="[
							'itbms-new-orders-button px-3 py-2 font-medium text-sm rounded-t-lg',
					...
					>
						...
						<div class="flex-grow space-y-2 w-full md:w-auto">
							...
							>
								...
								>
									Price: {{ formatNumber(item.price * item.quantity) }}
								</div>
							</div>
						</div>
