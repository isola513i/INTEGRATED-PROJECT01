...
const handleLogOut = () => {
  auth.logout();
  // ใช้ชื่อรูทให้ตรงกับโปรเจกต์ (ถ้าใช้ path ให้เปลี่ยนเป็น router.push('/sale-items'))
...
<template>
  <nav class="bg-[#171717] w-full z-50 text-white py-3">
    <div class="w-full relative flex items-center justify-between px-4 sm:px-6 lg:px-8">
      <!-- Logo -->
      ...
      <div class="absolute left-1/2 transform -translate-x-1/2 hidden md:flex space-x-6 lg:space-x-10">
        <router-link
          to="/sale-items"
      ...
      <div class="flex items-center gap-3 flex-shrink-0">
        ...
        <div class="hidden md:flex items-center space-x-4">
          <!-- Cart Icon -->
          <button class="hover:text-gray-400 transition-colors duration-200" aria-label="Cart">
          ...
          <template v-if="isSignedIn">
            <router-link
              :to="{ name: 'ProfileView' }"
              class="hover:text-gray-300 transition-colors duration-200 font-medium"
            >
              Profile
            </router-link>
            ...
            <button
              @click="handleLogOut"
              class="hover:text-gray-300 transition-colors duration-200 cursor-pointer"
            >
              Log out
            </button>
          </template>
          <template v-else>
            <button @click="goToSignIn" class="hover:text-gray-300 transition-colors duration-200 cursor-pointer">
              Sign in
            </button>
            <button @click="goToRegister" class="hover:text-gray-300 transition-colors duration-200 cursor-pointer">
              Register
            </button>
          </template>
        </div>
    ...
    <!-- Mobile Menu -->
    <div v-if="isOpen" class="md:hidden px-4 sm:px-6 lg:px-8 mt-3 space-y-1 pb-3 border-t border-gray-600">
      <!-- Mobile Search -->
      ...
      <template v-if="isSignedIn">
        <button
          @click="goToProfile"
          class="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-white hover:bg-gray-700 transition-colors duration-200"
        >
          Profile
        </button>
        <button
