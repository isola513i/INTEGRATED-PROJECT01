<script setup>
import { setDetail } from "@/services/cartService";
...
const props = defineProps({
  ...
  loading: { type: Boolean, default: false }, // ใช้ตอนกด Place Order
  savingDetail: { type: Boolean, default: false }, // ใช้ตอนกด Save Detail
});
...
// sync กลับ parent (v-model style)
...
// กดบันทึก address / note ลง cart (เรียก API PUT /v2/cart/{userId}/detail)
async function onSaveDetail() {
  if (!addressRequired.value || props.savingDetail) return;
  await setDetail(userStore.user.id, address.value, note.value);
}
</script>
  ...
  >
    ...
    <section class="divide-y divide-gray-200">
      ...
      <div class="pb-4">
        ...
        <div class="mt-4">
          <button
            class="itbms-save-detail-button w-full rounded-lg border border-gray-300 bg-white text-gray-800 text-sm font-medium px-3 py-2 shadow-sm hover:bg-gray-50 hover:text-black focus:ring-2 focus:ring-black/20 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-white disabled:hover:text-gray-800"
            :disabled="!addressRequired || savingDetail"
            @click="onSaveDetail"
          >
            <span v-if="savingDetail">Saving...</span>
            <span v-else>Save Detail</span>
          </button>
        </div>
      </div>
